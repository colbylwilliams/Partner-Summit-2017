	...
	public partial class AzureClient // Pull
	{
		...
		where T : AzureEntity, new()
		{
			...
			try
			{
				...
				string queryId = $"{table?.TableName ?? typeof (T).Name}";

				if (useAnonymousUserTableAccess)
				{
					query = table?.Where (t => t.anonymousAccessUserId == anonymousAccessUserId);

					if (where != null)
					{
						query = query.Where (where);
						//System.Diagnostics.Debug.WriteLine ($"{where.Name}");
					}

					queryId = $"{queryId}{anonymousAccessUserId}";
				}
				else
				{
					query = where == null ? table?.CreateQuery () : table?.Where (where);

					queryId = $"{queryId}{syncQueryFormat}";
				}
