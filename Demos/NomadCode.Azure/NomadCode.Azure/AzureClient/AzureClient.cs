...
namespace NomadCode.Azure
{
	public partial class AzureClient
	{
		...
		static bool useAnonymousUserTableAccess { get; set; }
		...
		public async Task InitializeAzync (string mobileAppUri, bool useSingleUserAnonymousAccess = false)
		{
			useAnonymousUserTableAccess |= useSingleUserAnonymousAccess;

			try
			{
				await initializeAzync (mobileAppUri);
		...
		async Task initializeAzync (string mobileAppUri)
		{
		...
		public void Initialize (string mobileAppUri, bool useSingleUserAnonymousAccess = false)
		{
			useAnonymousUserTableAccess |= useSingleUserAnonymousAccess;

			return initialize (mobileAppUri);
		}


		void initialize (string mobileAppUri)
		{
			if (string.IsNullOrWhiteSpace (mobileAppUri))
			{
				throw new ArgumentException ("Cannot be null, empty, or whitespace.", nameof (mobileAppUri));
